...
class EncDecBaseConfig(FairseqDataclass):
    ...
    ff_expert_dim: int = field(
        default=2048, metadata={"help": "embedding dimension for FFN"}
    )
    attn_expert_dim: int = field(
        default=256,
        metadata={
            "help": "attention expert dimension used in each layer.",
        },
    )
    ...
    attn_num_expert: int = field(
        default=12,
        metadata={
            "help": "number of experts used in each layer (Attn.).",
        },
    )
    attn_k: int = field(default=8, metadata={"help": "attention top-k"})

    ff_num_expert: int = field(
        default=12,
        metadata={
            "help": "number of experts used in each layer (MLP).",
        },
    )
    ff_k: int = field(default=8, metadata={"help": "MLP top-k"})

    halting: bool = field(
        default=False,
        metadata={"help": "Dynamic halting mechanism for UT"}
    )
...
class TransformerConfig(FairseqDataclass):
    ...
    gating_dropout: float = field(
        default=0.0,
        metadata={
            "help": "dropout probability after activation in moe gating.",
        },
    )
    halting_dropout: float = field(
        default=0.0,
        metadata={
            "help": "dropout probability after activation in halting.",
        },
    )

    moe_gumbel: bool = field(
        default=False, metadata={"help": "gumbel noise for expert selector"}
    )

    head_dim: int = field(
        default=64,
        metadata={
            "help": "head dimension used in each layer.",
        },
    )
    sample_topk: int = field(
        default=0,
        metadata={
            "help": "Number of sampled experts.",
        },
    )
    cvloss: float = field(
        default=0.0,
        metadata={
            "help": "weight for cv loss.",
        },
    )
    switchloss: float = field(
        default=0.0,
        metadata={
            "help": "weight for switch loss.",
        },
    )
    zloss: float = field(
        default=0.0,
    ...
    actloss: float = field(default=0.0, metadata={"help": "weight for ACT (dynamic halting) loss"})
