...
class MultiheadAttention(nn.Module):
    ...
    def __init__(
        ...
        self.k_proj = quant_noise(
            nn.Linear(self.kdim, self.expert_dim, bias=bias), q_noise, qn_block_size
        )
        self.v_proj = quant_noise(
            nn.Linear(self.vdim, self.expert_dim, bias=bias), q_noise, qn_block_size
        )
        self.q_proj = quant_noise(
            MoE(embed_dim, self.expert_dim, num_expert, k=top_k,
    ...
    def forward(
        ...
        if incremental_state is not None:
            saved_state = self._get_input_buffer(incremental_state, layer_idx)
        ...
        if self.self_attention:
            attn_weights = attn_weights + self.rel_pos_logits(
                q,
                src_len, 
                last=(incremental_state is not None)
        ...
        if key_padding_mask is not None:
            ...
            attn_weights = attn_weights.view(bsz, self.top_k, self.num_heads, tgt_len, src_len)
            attn_weights = attn_weights.masked_fill(
                key_padding_mask[:, None, None, None, :].to(torch.bool),
                float("-inf"),
            )
            attn_weights = attn_weights.view(total_bsz, tgt_len, src_len)
    ...
    def reorder_incremental_state(
        ...
        layer_idx = 0
        while True:
            input_buffer = self._get_input_buffer(incremental_state, layer_idx,
                                                  create_empty=False)
    ...
    def _get_input_buffer(
        ...
        layer_idx, create_empty=True
    ) -> Dict[str, Optional[Tensor]]:
        result = self.get_incremental_state(incremental_state, "attn_state_%d" % layer_idx)
    ...
    def _set_input_buffer(
        self,
        incremental_state: Dict[str, Dict[str, Optional[Tensor]]],
        buffer: Dict[str, Optional[Tensor]], layer_idx
    ):
        return self.set_incremental_state(
            incremental_state, "attn_state_%d" % layer_idx, buffer)
